/*
 *      $Id: xdr_rec.c,v 4.2 1994/09/29 23:48:50 jraja Exp $
 *
 *      Implements TCP/IP based XDR streams with a "record marking"
...
static bool_t flush_out(RECSTREAM * rstrm, int eor);
...
xdrrec_create(xdrs, sendsize, recvsize, tcp_handle, readit, writeit)
	...
	void * tcp_handle;
	int (*readit)(void *, caddr_t, int);  /* like read, but pass it a tcp_handle, not sock */
	int (*writeit)(void *, caddr_t, int);  /* like write, but pass it a tcp_handle, not sock */
{
...
#ifdef AMITCP
	/*
	 * Calling lseek() on a opaque tcp_handle is doomed to fail. In 
	 * addition, lseek cannot be performed on a socket, so why 
	 * bother to call the lseek() at all?
	 */
	pos = -1;
	fprintf(stderr, "xdr_rec.c: someone called xdrrec_getpos()!");
#else
